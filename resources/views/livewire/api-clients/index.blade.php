<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Manajemen API Key</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Kelola API key untuk aplikasi yang mengakses API referensi</p>
        </div>
    ...
    <div class="flex items-center space-x-4">
        <div class="flex-1 relative">
            ...
            <input 
                ...
                placeholder="Cari API client (nama, deskripsi)..."
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($client->ip_whitelist)
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300" title="{{ $client->ip_whitelist }}">
                                        {{ \Illuminate\Support\Str::limit($client->ip_whitelist, 30) }}
                                    </span>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">
                                    {{ number_format($client->request_count) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                {{ $client->last_used_at ? $client->last_used_at->format('d/m/Y H:i') : 'Belum pernah' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <button 
                                        ...
                                        wire:confirm="Apakah Anda yakin ingin menghapus API client '{{ $client->name }}'?"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                @if($search)
                                    Tidak ada API client yang ditemukan dengan kata kunci "{{ $search }}"
                                @else
                                    Belum ada API client. <a href="{{ route('api-clients.create') }}" class="text-blue-600 hover:text-blue-800 dark:text-blue-400">Buat yang pertama</a>
                                @endif
                            </td>
                        </tr>
                    @endforelse
